...
RUN \
  yum install -y cairo-devel libXt-devel opencv-devel gperftools-devel cmake
...
RUN \
  make -j$(nproc --ignore=1) USE_OPENCV=1 USE_BLAS=mkl USE_LAPACK=1 USE_GPERFTOOLS=1 USE_MKLDNN=1 USE_CUDA=1 USE_CUDA_PATH=$CUDA_HOME USE_CUDNN=1
